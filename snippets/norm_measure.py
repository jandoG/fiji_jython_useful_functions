...
#		imp.show();
		...
		if hepa_roi is None:
			print "No hepatocytes found in current image ", impName, " moving to next image"
			continue 
		...
		ch1_hepaNorm = normalizeToRef(measure_ch, scale, offset)
#		ch1_hepaNorm.show()
...
def normalizeToRef( imp0, scale, offset):
	imp = Duplicator().run(imp0)

	IJ.run(imp, "Multiply...", "value=" + str(scale));
	IJ.run(imp, "Add...", "value=" + str(offset));
...
#	imp1 = Duplicator().run(imp1)
	imp.setCalibration( imp0.getCalibration() )
	imp.setTitle("norm - " + imp0.getTitle())
