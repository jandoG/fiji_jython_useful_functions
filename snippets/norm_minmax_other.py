...
#
#		# set cytoplasm roi on channel 1 and get min and max
#		measure_ch.setRoi(cyto_roi)
#		stats_cyto = measure_ch.getAllStatistics()
#		new_range = float(stats_cyto.max - stats_cyto.min)
#		measure_ch.killRoi()
#		
...
#
#		# set on channel 1 and measure min and max 
#		measure_ch.setRoi(hepa_roi)
#		stats_hepa = measure_ch.getAllStatistics()
#		old_range = float(stats_hepa.max - stats_hepa.min)
#		measure_ch.killRoi()
#
#		scale = (stats_cyto.max - stats_cyto.min)/ float(stats_hepa.max - stats_hepa.min)
#		offset = stats_cyto.min - stats_hepa.min * scale 
#
#		ch1_hepaNorm = normalizeToRef(measure_ch, scale, offset, hepa_roi)
##		ch1_hepaNorm.show()
#		ch1_hepaNorm.setRoi(hepa_roi)
#		ch1_hepa_mean_norm = ch1_hepaNorm.getStatistics().mean
#		print "ch1_hepaNorm mean =",  ch1_hepa_mean_norm
##		measure_ch.killRoi()
#				
##		nuclei_oldMean = stats_hepa.mean 
##		nuclei_newMean = nuclei_oldMean - stats_cyto.min * (new_range/ old_range) + stats_cyto.min
##		nuclei_newMean1 = nuclei_oldMean * scale + offset
##		print(nuclei_newMean, nuclei_newMean1)
#
#
#		table.incrementCounter();
#		table.addValue("Name", impName);
#		table.addValue("Nuclei min", stats_hepa.min);
#		table.addValue("Nuclei max", stats_hepa.max);
#		table.addValue("Cytoplasm min", stats_cyto.min);
#		table.addValue("Cytoplasm max", stats_cyto.max);
#		table.addValue("Nuclei mean old", stats_hepa.mean);
#		table.addValue("Nuclei mean new scaled wrt cyto", ch1_hepa_mean_norm);
...
#		measure_ch.show()
		...
		# get mask of the hepatocytes
		hepa_mask = processImp(imp)
		IJ.run(hepa_mask, "Create Selection", "");
		hepa_roi = hepa_mask.getRoi() 

		impsrc = measure_ch.duplicate()
		imptrgt = measure_ch.duplicate()

		imp_c1Norm = getminmaxnormalized(impsrc, cyto_roi, imptrgt)

		measure_ch.setRoi(hepa_roi)
		oldmean = measure_ch.getStatistics().mean 

#		imp_c1Norm.show()
		imp_c1Norm.setRoi(hepa_roi)
		newmean = imp_c1Norm.getStatistics().mean 

		print oldmean, newmean

		table.incrementCounter();
		table.addValue("Name", impName);
		table.addValue("Nuclei mean old", oldmean);
		table.addValue("Nuclei mean new scaled wrt cyto", newmean);
...
def getminmaxnormalized(impSrc, cyto_roi, impTrgt):
	'''
	...
	'''
	IJ.run(impTrgt, "32-bit", "");
	
	impSrc.setRoi(cyto_roi)
	stats = impSrc.getAllStatistics()
	minI = stats.min
	maxI = stats.max
	Irange = maxI-minI

	img = ImageJFunctions.wrap(impTrgt)
	img_norm = ops.math().subtract(img, minI)
	img_norm = ops.math().divide(img, Irange)
	img_norm = ops.math().multiply(img, 65535)

	imp_norm = ImageJFunctions.wrap(img_norm, "minmax_norm - " + os.path.splitext(impSrc.getTitle())[0])
	imp_norm.setCalibration( impSrc.getCalibration() )
	imp_norm.setTitle("minmax_norm - " + os.path.splitext(impSrc.getTitle())[0])
